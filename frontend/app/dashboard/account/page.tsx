...
export default function UserDataPage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        setProfile({
          name: `${userData.firstName || ''} ${userData.lastName || ''}`.trim() || clerkUser.fullName || '',
          ...
          address: '123 Banking Street, New York, NY 10001',
          dateOfBirth: '1990-05-15',
          accountNumber: userData.accountNumber || '******3278',
          customerId: userData.customerId || 'CUST-123456',
          memberSince: userData.memberSince || 'January 2024'
        })
      } catch (err: any) {
        console.error('Failed to fetch user data from backend, using Clerk data as fallback:', err)

        // Silently fallback to Clerk data only
        if (clerkUser) {
          setProfile({
  ...
  const statisticsCardData = [
    ...
    },
    {
      ...
      value: profile.accountNumber || '******3278',
    ...
    },
    {
      ...
      value: 'Premium',
      title: 'Account Type',
      changePercentage: profile.memberSince || 'January 2024'
    },
    {
      icon: <CalendarIcon className='size-4' />,
      value: profile.customerId || 'CUST-123456',
      title: 'Customer ID',
      changePercentage: 'Verified'
    }
  ]
